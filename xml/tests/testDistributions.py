...
def addBinnedDistributionBin(distr, min_value, max_value, count):
    bin_element = etree.SubElement(distr, BinnedDistributionConstants.BIN_TAG)
    bin_element.attrib[BinnedDistributionConstants.BIN_MIN_VALUE_ATTR] = str(min_value)
    bin_element.attrib[BinnedDistributionConstants.BIN_MAX_VALUE_ATTR] = str(max_value)
    bin_element.attrib[BinnedDistributionConstants.BIN_COUNT_ATTR] = str(count)
...
def addEmpiricalDataPoint(distribution_node, probability=None, value=None):
    ret = etree.SubElement(distribution_node, EmpiricalDistributionConstants.DATA_POINT_TAG)
...
def createNormalDistributionNode(mean, sd, min_value=None, max_value=None):
    ...
    if min_value is not None:
        e.attrib[NormalDistributionConstants.MIN_VALUE_ATTR] = str(min_value)
    if max_value is not None:
        e.attrib[NormalDistributionConstants.MAX_VALUE_ATTR] = str(max_value)
...
def createEmpiricalDistributionNode(values_tuples):
    e = etree.Element(EmpiricalDistributionConstants.TAG)
    for mapping in values_tuples:
...
def createBinnedDistributionNode(values_tuples, aggression):
...
def createRawEmpiricalDistributionNode(values, aggression_direction):
    e = etree.Element(RawEmpiricalDistributionConstants.TAG)
    e.attrib[RawEmpiricalDistributionConstants.AGGRESSION_ATTR] = aggression_direction
    for value in values:
...
def addVehicleModelShare(distribution_node, occurence, value):
...
def createVehicleModelsDistributionNode(attach_to, share_tuples_as_occurrence_value, name='vehicle model distribution'):
    ret = etree.SubElement(
        attach_to,
    ...
    for shareTuple in share_tuples_as_occurrence_value:
...
def createAndAddColorDistributionNode(attach_to, share_tuples_as_occurrence_value, name='a color distribution'):
    ret = etree.SubElement(attach_to, ColorDistributionConstants.TAG, {
...
def createAndAddCleanColorDistributionNode(attach_to):
    return createAndAddColorDistributionNode(attach_to, [
        [1, '#ff0000', ],
        [2, '#ff8000', ],
        [3, '#ffff00', ],
        [4, '#00ff00', ],
...
def create_and_add_acceleration_distribution_node(attach_to, dp_tuples_as_v_m_sd, speed_unit, accel_unit,
                                                  name='an accel distribution'):
    main_attributes = {
...
def create_and_add_distance_distribution_node(attach_to, units: str, distr_node: etree.Element,
...
def create_and_add_speed_distribution_node(attach_to, units: str, distr_node: etree.Element,
                                           name: str = 'Unnamed Speed Distribution'):
    ret = etree.SubElement(attach_to, 'distribution', {
...
def create_and_add_posted_speed_deviation_distribution_node(attach_to, speed_units, distr,
                                                            name='posted speed deviation distribution'):
...
class CleanDistributionsDocument:
    def __init__(self):
        ...
        self.documentRoot = etree.Element('distributions', nsmap=ns_map)
        self.documentRoot.attrib[
            '{http://www.w3.org/2001/XMLSchema-instance}noNamespaceSchemaLocation'] = '../distributions.xsd'

        self.connectorLinkSelectionBehaviorsNode = etree.SubElement(self.documentRoot, DistributionSetConstants.TAG)
        self.connectorLinkSelectionBehaviorsNode.attrib[
            DistributionSetConstants.TYPE_ATTR] = 'connector-link-selection-behaviors'
        ...
        self.connectorMaxPositioningDistancesNode.attrib[
            DistributionSetConstants.TYPE_ATTR] = 'connector-max-positioning-distance'
        first_normal_dist = createNormalDistributionNode(1609, 402, min_value=400)
        first_normal_dist.attrib['median'] = '1600'
        create_and_add_distance_distribution_node(self.getConnectorMaxPositioningDistancesNode(), DistanceUnits.FEET,
                                                  first_normal_dist)
        normal_distribution_node = createNormalDistributionNode(804.5, 201, min_value=300, max_value=1000)
        create_and_add_distance_distribution_node(self.getConnectorMaxPositioningDistancesNode(), DistanceUnits.FEET,
                                                  normal_distribution_node)
        create_and_add_distance_distribution_node(self.getConnectorMaxPositioningDistancesNode(), DistanceUnits.FEET,
                                                  createEmpiricalDistributionNode(
                                                      [(0, 2000), (0.15, 1500), (0.85, 800), (1, 400)]),
                                                  'observed by advanced technology')
        values = [1525.1118, 2202.5331, 1257.0525, 1577.4787, 831.2836,
                  ...
                  1756.4594]
        node = createRawEmpiricalDistributionNode(values, RawEmpiricalDistributionConstants.AGGRESSION_VALUE_NEGATIVE)
        create_and_add_distance_distribution_node(self.getConnectorMaxPositioningDistancesNode(), DistanceUnits.FEET,
                                                  node, 'observed lc distances')
        bins = [(0, 10, 2), (10, 20, 5), (20, 30, 10), (30, 40, 6), (40, 50, 3)]
        distribution_node = createBinnedDistributionNode(bins, BinnedDistributionConstants.AGGRESSION_VALUE_NEGATIVE)
        create_and_add_distance_distribution_node(self.getConnectorMaxPositioningDistancesNode(), DistanceUnits.FEET,
                                                  distribution_node)
        ...
        createVehicleModelsDistributionNode(self.vehicleModelsNode, [
    ...
    def validate(self) -> bool:
        xs_doc = etree.parse('../distributions.xsd')
...
class TestsForSimpleDataTypes(unittest.TestCase):
    ...
    def testThatUuidsAreBeingValidated(self):
        distr = createCleanNormalDistributionNode()
        node = create_and_add_distance_distribution_node(self.doc.getConnectorMaxPositioningDistancesNode(),
                                                         DistanceUnits.FEET, distr)
    ...
    def testThatStandardDeviationsAreBeingValidated(self):
        ...
        create_and_add_distance_distribution_node(self.doc.getConnectorMaxPositioningDistancesNode(),
                                                  DistanceUnits.FEET, node)
        self.assertFalse(self.doc.validate())
...
class TestsForBinnedDistributions(unittest.TestCase):
    ...
    def testThatBinSubElementsAreBanned(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.append(etree.Element('a-sub-element'))
        self.append(node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatOtherBinAttributesAreOkay(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib['bin-name'] = 'aggressive folks'
        self.append(node)
        self.assertTrue(self.doc.validate())
    ...
    def testThatMinValueIsRequired(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib.pop(BinnedDistributionConstants.BIN_MIN_VALUE_ATTR)
        self.append(node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatMinValueMustBeNumeric(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib[BinnedDistributionConstants.BIN_MIN_VALUE_ATTR] = 'not a number'
        self.append(node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatMaxValueIsRequired(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib.pop(BinnedDistributionConstants.BIN_MAX_VALUE_ATTR)
        self.append(node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatMaxValueMustBeNumeric(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib[BinnedDistributionConstants.BIN_MAX_VALUE_ATTR] = 'not a number'
        self.append(node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatCountIsRequired(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib.pop(BinnedDistributionConstants.BIN_COUNT_ATTR)
        self.append(node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatCountMustBeNumeric(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib[BinnedDistributionConstants.BIN_COUNT_ATTR] = 'not a number'
        self.append(node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatCountMustBeNonnegative(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib[BinnedDistributionConstants.BIN_COUNT_ATTR] = '-5'
        self.append(node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatCountMayBeZero(self):
        node = createCleanBinnedDistributionNode()
        first_bin = node[0]
        first_bin.attrib[BinnedDistributionConstants.BIN_COUNT_ATTR] = '0'
        self.append(node)
        self.assertTrue(self.doc.validate())
...
class TestsForRawEmpiricalDistributions(unittest.TestCase):
    ...
    def testThatObservationSubElementsAreBanned(self):
        node = self.createCleanDistributionNode()
        dp_node = addRawEmpiricalDistributionObservation(node, 1234)
        etree.SubElement(dp_node, 'another-subelement')
        create_and_add_distance_distribution_node(self.doc.getConnectorMaxPositioningDistancesNode(),
                                                  DistanceUnits.FEET, node)
        self.assertFalse(self.doc.validate())
    ...
    def testThatOtherObservationAttributesAreOkay(self):
        node = self.createCleanDistributionNode()
        dp_node = addRawEmpiricalDistributionObservation(node, 1234)
        dp_node.attrib['another-attr'] = 'okay!'
        create_and_add_distance_distribution_node(self.doc.getConnectorMaxPositioningDistancesNode(),
                                                  DistanceUnits.FEET, node)
        self.assertTrue(self.doc.validate())
    ...
    def testThatValueIsRequired(self):
        node = self.createCleanDistributionNode()
        dp_node = addRawEmpiricalDistributionObservation(node)
        dp_node.attrib['other-attr'] = 'blah'
        create_and_add_distance_distribution_node(self.doc.getConnectorMaxPositioningDistancesNode(),
                                                  DistanceUnits.FEET, node)
        self.assertFalse(self.doc.validate())
...
class TestsForDistanceDistributions(unittest.TestCase):
    ...
    def test_that_name_is_optional(self):
        distr = createCleanNormalDistributionNode()
        node = create_and_add_distance_distribution_node(self.test_root, DistanceUnits.FEET, distr,
                                                         'normal distribution')
    ...
    def test_that_normal_distribution_is_ok(self):
        distr = createCleanNormalDistributionNode()
        create_and_add_distance_distribution_node(self.test_root, DistanceUnits.FEET, distr,
                                                  'normal distribution')
        self.assertTrue(self.doc.validate())
    ...
    def test_that_raw_empirical_distribution_is_ok(self):
        distr = createRawEmpiricalDistributionNode([1, 2, 3, 4],
                                                   RawEmpiricalDistributionConstants.AGGRESSION_VALUE_POSITIVE)
        create_and_add_distance_distribution_node(self.test_root, DistanceUnits.FEET, distr,
    ...
    def test_that_binned_distribution_is_ok(self):
        ...
        create_and_add_distance_distribution_node(self.test_root, DistanceUnits.FEET, distr, 'binned distribution')
        self.assertTrue(self.doc.validate())
...
class TestsForConnectorLinkSelectionBehaviorDistributions(unittest.TestCase):
    ...
    def testThatShareMustBeNumeric(self):
        addEnumDistributionShare(self.cleanDistr, 'non numeric!',
                                 ConnectorLinkSelectionBehaviorDistributionConstants.BEST)
        self.doc.getConnectorLinkSelectionBehaviorsNode().append(self.cleanDistr)
        self.assertFalse(self.doc.validate())
...
class TestsForConnectorMaxPositioningDistance(unittest.TestCase):
    ...
    def testThatRawEmpiricalDistributionCanBeAdded(self):
        node = self.doc.getConnectorMaxPositioningDistancesNode()
        distr = createRawEmpiricalDistributionNode([3, 4, 5, 6],
                                                   RawEmpiricalDistributionConstants.AGGRESSION_VALUE_POSITIVE)
        create_and_add_distance_distribution_node(node, DistanceUnits.FEET, distr)
        self.assertTrue(self.doc.validate())
...
class TestsForVehicleModelDistributions(unittest.TestCase):
    def setUp(self):
        self.doc = CleanDistributionsDocument()
        self.cleanDistr = createVehicleModelsDistributionNode(self.doc.getVehicleModelsNode(), [
            (35, '47e301a2-f905-4245-94ef-127daf6736b7'),
            (30, '9c344e0f-30cc-44e9-ba3b-b3c1b6137bda'),
            (20, '223367cb-69a6-474c-b55f-2405f0c1ba18')
        ])
    ...
    def testThatValueIsRequired(self):
        first_node = self.cleanDistr[0]
        first_node.attrib.pop(VehicleModelDistributionConstants.SHARE_VALUE_ATTR)

        self.assertFalse(self.doc.validate())

    def testThatValueMayNotBeEmpty(self):
        first_node = self.cleanDistr[0]
        first_node.attrib[VehicleModelDistributionConstants.SHARE_VALUE_ATTR] = ''

        self.assertFalse(self.doc.validate())

    def testThatValueMayNotBeArbitraryString(self):
        first_node = self.cleanDistr[0]
        first_node.attrib[VehicleModelDistributionConstants.SHARE_VALUE_ATTR] = 'not a uuid'

        self.assertFalse(self.doc.validate())

    def testThatValueMayNotBeArbitraryNumber(self):
        first_node = self.cleanDistr[0]
        first_node.attrib[VehicleModelDistributionConstants.SHARE_VALUE_ATTR] = '12345'
...
class TestsForColorDistributions(unittest.TestCase):
    ...
    def testThatShareSubElementsAreBanned(self):
        node = createAndAddCleanColorDistributionNode(self.targetNode)
        first_share = node[0]
        etree.SubElement(first_share, 'a-child-node')
        self.assertFalse(self.doc.validate())

    def testThatOtherShareAttributesAreOkay(self):
        node = createAndAddCleanColorDistributionNode(self.targetNode)
        first_share = node[0]
        first_share.attrib['another-attribute'] = 'value'
        self.assertTrue(self.doc.validate())
...
class TestsForAccelerationDistributions(unittest.TestCase):
    ...
    def test_that_uuid_must_be_a_uuid(self):
        ...
        self.assertFalse(self.doc.validate())
        node.attrib[MaxAccelerationDistributionConstants.UUID_ATTR] = 'c3937207-d6db-481g-998e-e3abec44abb2'
        self.assertFalse(self.doc.validate())
...
class TestsForMaxDecelerations(unittest.TestCase):
    ...
    def test_that_uuid_is_being_verified(self):
        ...
        self.assertFalse(self.doc.validate())
        node.attrib[
            MaxDecelerationDistributionConstants.UUID_ATTR] = 'b8ce2cf0-a220-41eb-835c-fd926032d450'  # proper uuid
        self.assertTrue(self.doc.validate())
    ...
    def test_that_normal_distribution_is_ok(self):
        ...
        create_and_add_max_decel_distribution_node(target, distr, AccelerationUnits.METERS_PER_SECOND_SQUARED)
        self.assertTrue(self.doc.validate())
    ...
    def test_that_raw_empirical_distribution_is_ok(self):
        distr = createRawEmpiricalDistributionNode([5, 6, 3, 6, 3, 7],
                                                   RawEmpiricalDistributionConstants.AGGRESSION_VALUE_POSITIVE)
        target = self.doc.get_max_decelerations_node()
        create_and_add_max_decel_distribution_node(target, distr, AccelerationUnits.METERS_PER_SECOND_SQUARED)
        self.assertTrue(self.doc.validate())
    ...
    def test_that_distribution_set_is_required(self):
        ...
        elements_to_remove = filter(
            lambda item: (item.attrib[DistributionSetConstants.TYPE_ATTR] ==
                          MaxDecelerationDistributionConstants.DISTRIBUTION_TYPE),
            distribution_sets)
        for element in elements_to_remove:
...
class TestsForDesiredAccelerationFractions(unittest.TestCase):
    ...
    def test_for_valid_distribution_types(self):
        distr_tuple_list = [
            ...
            (createEmpiricalDistributionNode([(0.0, 0.5), (1.0, 0.8)]), True),
            # the fractional empirical distr is just a special case
            # where values are in the range [0, 1]
            (createEmpiricalDistributionNode([(0.0, 0.5), (1.0, 1.2)]), False),
            # because there is a value outside the range [0, 1]
            (createBinnedDistributionNode([(0, 1, 5)], BinnedDistributionConstants.AGGRESSION_VALUE_POSITIVE), False),
            (createRawEmpiricalDistributionNode([0.4, 0.4, 0.6],
                                                RawEmpiricalDistributionConstants.AGGRESSION_VALUE_POSITIVE), False),
        ]

        for (distribution, expected_result) in distr_tuple_list:
            self.target_node[:] = []
            create_and_add_desired_accel_fraction_node(self.target_node, distribution)
    ...
    def test_that_normal_distribution_reverse_is_ok(self):
        # Because accel fraction distributions that are not monotonic-increasing will generate simulator warnings,
    ...
    def test_that_distribution_set_is_required(self):
        ...
        elements_to_remove = filter(
            lambda item: (item.attrib[DistributionSetConstants.TYPE_ATTR] ==
                          DesiredAccelerationFractionDistributionConstants.DISTRIBUTION_TYPE),
            distribution_sets)
        for element in elements_to_remove:
            self.doc.getDocumentRoot().remove(element)
        self.assertFalse(self.doc.validate())
    ...
    def test_that_distribution_count_may_be_one_thousand(self):
        for _ in range(1, 1000):
            create_and_add_clean_desired_accel_fraction_node(self.target_node)
...
class TestsForDesiredDecelFractionDistributions(unittest.TestCase):
    ...
    def test_that_distribution_set_is_required(self):
        ...
        elements_to_remove = filter(
            lambda item: (item.attrib[DistributionSetConstants.TYPE_ATTR] ==
                          DesiredDecelerationFractionDistributionConstants.DISTRIBUTION_TYPE),
            distribution_sets)
        for element in elements_to_remove:
            self.doc.getDocumentRoot().remove(element)
        self.assertFalse(self.doc.validate())
    ...
    def test_that_distribution_count_may_be_one_thousand(self):
        for _ in range(1, 1000):
            create_and_add_clean_desired_decel_fraction_node(self.target_node)
    ...
    def test_for_valid_distribution_types(self):
        ...
        for (distribution, expected_result) in distr_tuple_list:
            self.target_node[:] = []
            create_and_add_desired_decel_fraction_node(self.target_node, distribution)
            self.assertEqual(expected_result, self.doc.validate())
...
class TestsForTargetSpeedDistributions(unittest.TestCase):
    ...
    def test_for_valid_distribution_types(self):
        distr_tuple_list = [
            ...
            # the fractional empirical distr is just a special case
            # where values are in the range [0, 1]
            (createEmpiricalDistributionNode([(0.0, 0.5), (1.0, 1.2)]), True),
            # because there is a value outside the range [0, 1]
            (createBinnedDistributionNode([(0, 1, 5)], BinnedDistributionConstants.AGGRESSION_VALUE_POSITIVE), True),
            (createRawEmpiricalDistributionNode([0.4, 0.4, 0.6],
                                                RawEmpiricalDistributionConstants.AGGRESSION_VALUE_POSITIVE), True),
        ]

        for (distribution, expected_result) in distr_tuple_list:
            self.target_node[:] = []
            create_and_add_speed_distribution_node(self.target_node, SpeedUnits.MILES_PER_HOUR, distribution)
            self.assertEqual(expected_result, self.doc.validate())
...
class TestsForPostedSpeedVariation(unittest.TestCase):
    def setUp(self) -> None:
        self.doc = CleanDistributionsDocument()
        self.target_node = self.doc.get_posted_speed_deviations_node()

    def test_that_posted_speed_deviations_node_is_required(self):
        distribution_sets = self.doc.getDocumentRoot().iter(DistributionSetConstants.TAG)
        elements_to_remove = filter(
            lambda item: (item.attrib[DistributionSetConstants.TYPE_ATTR] ==
                          PostedSpeedDeviationConstants.DISTRIBUTION_TYPE),
            distribution_sets)
        for element in elements_to_remove:
            self.doc.getDocumentRoot().remove(element)
        self.assertFalse(self.doc.validate())
    ...
    def test_that_name_is_optional(self):
        node = create_and_add_clean_posted_speed_deviation_distribution_node(self.target_node)
        node.attrib.pop(PostedSpeedDeviationConstants.NAME_ATTR)
        self.assertTrue(self.doc.validate())

    def test_values_for_name(self):
        node = create_and_add_clean_posted_speed_deviation_distribution_node(self.target_node)
        valid_names = ['', 'distribution name', '1102']
        for name in valid_names:
            node.attrib[PostedSpeedDeviationConstants.NAME_ATTR] = name
            self.assertTrue(self.doc.validate())

    def test_that_uuid_is_required(self):
        node = create_and_add_clean_posted_speed_deviation_distribution_node(self.target_node)
        node.attrib.pop(PostedSpeedDeviationConstants.UUID_ATTR)
        self.assertFalse(self.doc.validate())
    ...
    def test_that_units_are_required(self):
        node = create_and_add_clean_posted_speed_deviation_distribution_node(self.target_node)
        node.attrib.pop(PostedSpeedDeviationConstants.UNITS_ATTR)
        self.assertFalse(self.doc.validate())
    ...
    def test_that_units_is_verified(self):
        ...
        for (value, expected_result) in test_tuples:
            node.attrib[PostedSpeedDeviationConstants.UNITS_ATTR] = value
            self.assertEqual(expected_result, self.doc.validate())

    def test_for_valid_distribution_types(self):
        distribution_tuple_list = [
            (create_normal_fractional_distribution_node(0.5, 0.15), True),
            (create_empirical_fractional_distribution_node([(0.0, 0.5), (1.0, 0.8)]), True),
            (createNormalDistributionNode(0.5, 0.15), True),
            (createEmpiricalDistributionNode([(0.0, 0.5), (1.0, 0.8)]), True),
        ...
        for (distribution, expected_result) in distribution_tuple_list:
            self.target_node[:] = []
            create_and_add_speed_distribution_node(self.target_node, SpeedUnits.MILES_PER_HOUR, distribution)
            self.assertEqual(expected_result, self.doc.validate())
